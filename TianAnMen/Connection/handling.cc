...
void handling::Handler(SOCKET socket) {
    ...
    if (recv(socket, (char *) buffer, PACKET_SIZE, 0) == -1) {
        client_list::Remove(socket);
#ifdef DEBUG
        wprintf(L"[DEBUG] SOCKET_ERROR_1\n");
#endif
        return;
    } else {
        struct PACKET *init_packet = (struct PACKET *) &buffer;

#ifdef DEBUG
        wprintf(L"[DEBUG] PACKET_TYPE: %d\n[DEBUG]PACKET_TASK_ID: %s\n", init_packet->type, init_packet->task_id);
#endif
        struct INFO *init_info = (struct INFO *) &init_packet->data;
    ...
    while (true) {
        ...
        packet::MakePing(&ping_packet);
        if (send(socket, (char *) &ping_packet, PACKET_SIZE, 0) == -1) {
            client_list::Remove(socket);
#ifdef DEBUG
            wprintf(L"[DEBUG] SOCKET_ERROR_2\n");
#endif
            return;
        } else {
#ifdef DEBUG
            ...
            if (recv(socket, (char *) buf, PACKET_SIZE, 0) == -1) {
                client_list::Remove(socket);
#ifdef DEBUG
                wprintf(L"[DEBUG] SOCKET_ERROR_3\n");
#endif
                return;
            } else {
#ifdef DEBUG
                ...
                receive_packet = (struct PACKET *) &buf;
#ifdef DEBUG
